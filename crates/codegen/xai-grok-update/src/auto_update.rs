...
pub async fn check_update_background(update_config: &UpdateConfig) -> BackgroundUpdateCheck {
    // AUTO-UPDATE HARD-DISABLED (local fork patch): no background update
    // checks — they phone home to the release channel and lead to in-place
    // replacement of this fork build. See run_update_if_available.
    let auto_update_disabled = true;
...
pub async fn run_update_if_available(
    ...
    // AUTO-UPDATE HARD-DISABLED (local fork patch).
    //
    // A self-update downloads an UPSTREAM binary and installs it in place —
    // silently replacing this fork build and every privacy patch with it. It
    // is also a remote-driven code-update channel, which this fork's threat
    // model forbids. Return "no update" unconditionally; updates arrive only
    // via the fork's weekly sync (PATCH.md). The original logic is preserved
    // below, unreachable, so it stays live for anyone who reverts this patch.
    let auto_update_disabled = true;
    if auto_update_disabled {
