...
async fn fetch_login_device_flow(cli_chat_proxy_base_url: &str) -> Option<bool> {
    // LOGIN-CONFIG BEACON NEUTERED (local fork patch).
    //
    // This is an unauthenticated pre-login GET that transmits the
    // `x-grok-agent-id` machine fingerprint (plus client version and mode), so
    // it tells xAI "this install is about to log in" before any account
    // exists. Returning `None` makes the caller keep its built-in loopback
    // default, so login behavior is unchanged but no pre-login phone-home
    // fires. The original request is preserved below, unreachable, so the
    // helpers stay live for anyone who reverts this patch — reverting re-arms
    // the fingerprinted beacon. See PATCH.md patch 2b.
    let beacon_disabled = true;
    if beacon_disabled {
        return None;
    }
...
mod tests {
    ...
    mod login_config {
        ...
        // LOGIN-CONFIG BEACON NEUTERED (local fork patch): asserts the real HTTP fetch, which the neutered body never performs.
        #[ignore = "fork patch 2b: login beacon neutered"]
        #[tokio::test]
