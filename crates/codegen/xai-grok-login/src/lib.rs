...
// Public-monorepo test fix (PATCH.md §6 "test-compile errors", 1.0.38 sync):
// gcloud-auth (via xai-file-utils) unifies jsonwebtoken's `aws_lc_rs` feature
// into this build graph while this crate enables `rust_crypto`, so the lib
// test binary links BOTH jsonwebtoken providers and jsonwebtoken 10 panics on
// first use ("Could not automatically determine the process-level
// CryptoProvider"). The prod binary pins a provider at startup via
// xai-grok-extra-ca; the test binary has no startup hook, and upstream's
// per-test-module `install_default()` calls don't cover every module
// (23 tests panicked on first crypto use). One process-wide install at test
// binary load, mirroring oidc::test_helpers::ensure_crypto_provider().
#[cfg(test)]
