...
pub fn is_fetch_enabled() -> bool {
    // MANAGED CONFIG FETCH HARD-DISABLED (local fork patch).
    //
    // The managed-config sync (GET {cli-chat-proxy}/deployment/config, every
    // 5 min and post-login) downloads org policy and layers it into the
    // effective config — including `[telemetry] otel_*` keys that enable the
    // external OTEL stream (see patch 5 in PATCH.md). It is default-on,
    // deliberately overlay-free (the doc comment above), and NOT covered by
    // the remote-fetch disable (patch 2): a server-push channel that can
    // re-enable telemetry with zero local opt-in — exactly the remote re-arm
    // this fork exists to prevent. No remote service may re-enable anything
    // here. Explicit user actions (`grok setup`) skip this gate and still
    // work. We still evaluate the original decision so the `policy.rs`
    // helpers stay live for anyone who reverts this patch — but the effective
    // result is `false`. See PATCH.md patch 6.
