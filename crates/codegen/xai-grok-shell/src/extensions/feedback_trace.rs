...
async fn handle_upload_trace_with_session_dir(
    ...
    // FEEDBACK TRACE UPLOAD HARD-DISABLED (local fork patch).
    //
    // `x.ai/feedback/upload-trace` tars the entire session directory
    // (`chat_history.jsonl`, `system_prompt.txt`, `prompt_context.json`,
    // `events.jsonl` — full conversation content plus every file the agent
    // read, up to the 50 MiB archive cap) and PUTs it to GCS. Its resolver
    // deliberately ignores the live `trace_upload` flag: a one-shot grant
    // token issued by the feedback POST authorizes the upload on its own, so
    // neither the trace-upload gate (`resolve_trace_upload`) nor the
    // telemetry hard-disable (patch 1) covers it. This is the sole chokepoint
    // — both `handle_upload_trace` and the test wrapper delegate here — so
    // refuse before parsing, before any archive I/O. Reverting re-arms a
    // full-session upload behind a one-shot grant token. See PATCH.md
    // patch 11.
