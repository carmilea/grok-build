...
async fn handle_share_session(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    // SESSION SHARING REMOVED (local fork patch).
    //
    // Sharing uploads the ENTIRE conversation (ExportedSession JSON) to a
    // signed GCS URL and publishes a public grok.com/build/{id} link — bulk
    // exfiltration of session context by design. Upstream re-enabled it in
    // 1.0.5 behind the remote `sharing_enabled` flag; remote flags never
    // arrive on this build (patch 2), but rather than rely on a gate staying
    // dead, the handler is stripped: refuse unconditionally. This is the sole
    // chokepoint — `handle()` above dispatches only `x.ai/share_session`
    // here, and `BackendClient::share_session` has no other caller. We patch
    // the handler rather than delete the file so weekly upstream syncs stay
    // conflict-light. Original body preserved below, unreachable, so the
    // code stays live for anyone who reverts this patch. See PATCH.md patch 8.
    let sharing_removed = true;
    if sharing_removed {
        return Err(acp::Error::invalid_params()
            .data("Session sharing is not available in this build"));
    }
