//! Stubbed remote session restore.
//!
//! EGRESS-AUDIT (fork): upstream ships this subsystem stubbed out in public
//! builds — every entry point `bail!(UNAVAILABLE)`. That is load-bearing for
//! this fork's threat model: remote session sync is the download half of a
//! pair, and a real implementation landing here would imply the upload half
//! (repo contents + conversation to `cli-chat-proxy.grok.com/v1/storage/*`)
//! landing with it. The `StorageClient` upload API it would call is compiled
//! but currently has zero live callers.
//!
//! If an upstream sync replaces this file with a real implementation, STOP and
//! audit the upload path before merging — do not treat it as a routine
//! feature. Verified by: `make audit` (checks 3 and 4).
