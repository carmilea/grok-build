...
) -> Result<(SessionInitResult, tokio::sync::oneshot::Receiver<()>), xai_grok_agent::AgentBuildError>
{
    ...
    let web_search_config = if disable_web_search {
        ...
        // FORK PATCH 9 (web search backends): Anthropic model entries keep
        // their credential in `extra_headers.x-api-key`, which does not feed
        // `cfg.api_key`. The AnthropicServerTool wire format never sends the
        // Bearer header, so an x-api-key header satisfies the credential gate
        // with an inert placeholder key.
        let header_authed = matches!(
            cfg.api_backend,
            xai_grok_sampling_types::ApiBackend::Messages
        ) && cfg.extra_headers.contains_key("x-api-key");
        let resolved_api_key = cfg
            .api_key
            .clone()
            .or_else(|| header_authed.then_some(String::new()));
        if let Some(api_key) = resolved_api_key {
            // FORK PATCH 9 (web search backends): route the in-process web_search
            // tool to the same protocol the configured search model speaks.
            let wire_format = match cfg.api_backend {
                xai_grok_sampling_types::ApiBackend::Responses => {
                    xai_grok_tools::implementations::web_search::WebSearchWireFormat::Responses
                }
                xai_grok_sampling_types::ApiBackend::ChatCompletions => {
                    xai_grok_tools::implementations::web_search::WebSearchWireFormat::MoonshotBuiltin
                }
                xai_grok_sampling_types::ApiBackend::Messages => {
                    xai_grok_tools::implementations::web_search::WebSearchWireFormat::AnthropicServerTool
                }
            };
