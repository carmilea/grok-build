...
fn byok_from_models(
    ...
    // BYOK SIDE-CALL BEARER XAI-ONLY (local fork patch): the key selected here
    // becomes the process-wide "side-call bearer" for xAI side services —
    // voice STT (WebSocket to api.x.ai /v1/stt), image/video generation
    // (api.x.ai /v1/images, /v1/videos), /tokenize-text, and the ACP bearer
    // export. With BYOK-only credentials upstream would send a third-party
    // (e.g. Moonshot) key to api.x.ai on /voice, Ctrl+Space, /imagine, or a
    // model-invoked image_gen call. Only entries whose effective API-key
    // endpoint is a trusted xAI host may contribute a side-call credential;
    // chat requests to the entry's own provider are unaffected. Reverting
    // re-arms the third-party key leak.
    let xai_side_call_credential = |m: &ModelEntry| {
        let url = m.api_base_url.as_deref().unwrap_or(&m.info.base_url);
        if crate::util::is_trusted_xai_https_url(url) {
            m.own_credential()
        } else {
            None
        }
    ...
    preferred
        ...
        .and_then(xai_side_call_credential)
        .or_else(|| models.get(current).and_then(xai_side_call_credential))
        .or_else(|| models.values().find_map(xai_side_call_credential))
}
...
impl MvpAgent {
    ...
    pub fn with_models(
        ...
        let has_xai_auth = auth_manager
            .current_or_expired()
            .is_some_and(|a| a.is_xai_auth());
        // EGRESS-AUDIT (fork): conversation writeback gate. When enabled, the
        // sync task POSTs full message bodies to `/sessions/{id}/data`
        // (`remote::client::save_session_data`) plus a metadata upsert — the
        // entire conversation, continuously, not just at turn end.
        //
        // Off unless ALL THREE hold; none of them is remotely settable:
        //   tui_mode            — TUI only, never headless/ACP
        //   relay_config_enabled— `[relay] enabled` / `GROK_RELAY_SYNC_ENABLED`
        //                         (defaults false, see load_relay_sync_enabled_sync)
        //   has_xai_auth        — real xAI credentials, so BYOK/third-party
        //                         providers can never trigger it
        //
        // If an upstream sync drops any conjunct, exposure widens silently.
        // Verified by: `make audit` ("relay sync requires tui+config+xai-auth").
    ...
    ///
    /// EGRESS-AUDIT (fork): this is the chokepoint for the per-turn artifact
    /// upload engine. Returning `None` here means no `PromptTraceContext` is
    /// ever constructed, which transitively disables every turn-end upload
    /// including the memory upload in `upload::trace`. Two guards must survive
    /// upstream syncs, in this order:
    ///   1. `is_data_collection_disabled()` — ZDR short-circuit
    ///   2. `!cfg.is_trace_upload_enabled()` — the config gate
    ///      (see `Config::resolve_trace_upload`, which is a SOFT gate)
    /// A sync that reorders these, or that resolves an upload method before
    /// consulting them, silently re-arms per-turn upload.
    /// Verified by: `make audit` (checks 2.1 and 2.2).
