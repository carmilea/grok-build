...
#[test]
// REMOTE FETCH HARD-DISABLED (local fork patch): settings_allow_access is the
// access-gate chokepoint and fails open unconditionally — a cached
// `allow_access = false` must not block when no verdict can ever arrive.
// Behavior intentionally changed.
#[ignore = "access gate fails open by local fork patch"]
...
// fork patches 1+11: "every gate is open" requires `is_telemetry_enabled()`,
// which patch 1 forces off, and the upload itself is refused by patch 11.
#[tokio::test]
#[serial_test::serial]
#[ignore = "feedback trace offer gate is closed by local fork patches 1 and 11"]
...
// FEEDBACK TRACE UPLOAD HARD-DISABLED (local fork patch): asserts a gate outcome reached past the handler's unconditional refusal.
#[ignore = "fork patch 11: feedback trace upload disabled"]
#[tokio::test]
#[serial_test::serial]
...
#[tokio::test]
// TELEMETRY HARD-DISABLED (local fork patch): `resolve_telemetry_mode` always
// returns `Disabled`, so `is_telemetry_enabled()` / product analytics can never
// be on. Behavior intentionally changed; see Config::resolve_telemetry_mode.
#[ignore = "telemetry hard-disabled by local fork patch"]
...
#[tokio::test]
// TELEMETRY HARD-DISABLED (local fork patch): `is_telemetry_enabled()` is now
// always false, so product analytics can never be on. Behavior intentionally
// changed; see Config::resolve_telemetry_mode.
#[ignore = "telemetry hard-disabled by local fork patch"]
...
#[tokio::test]
#[serial_test::serial]
// REMOTE FETCH HARD-DISABLED (local fork patch): precondition "remote fetch
// is on" cannot hold on this build; remote answers never arrive. Behavior
// intentionally changed.
#[ignore = "remote fetch hard-disabled by local fork patch"]
...
#[serial_test::serial]
// REMOTE FETCH HARD-DISABLED (local fork patch): precondition "remote fetch
// is on" cannot hold on this build; remote settings never land. Behavior
// intentionally changed.
#[ignore = "remote fetch hard-disabled by local fork patch"]
...
#[serial_test::serial]
// REMOTE FETCH HARD-DISABLED (local fork patch): precondition "remote fetch
// is on" cannot hold on this build; the fetch lifecycle this test exercises
// cannot happen. Behavior intentionally changed.
#[ignore = "remote fetch hard-disabled by local fork patch"]
...
// fork note (1.0.38 sync): new upstream test asserts the process-global
// `STAMPS_SERVED` counter, which sibling tests bump concurrently — flaky
// under the fast gate's parallelism; passes in isolation.
#[test]
#[ignore = "flaky under full-suite parallelism (global stamp counters); passes in isolation"]
...
// fork note (1.0.38 sync): new upstream test — ambient `~/.grok` MCP config
// keeps the withdrawn install's toolset alive (fails deterministically with a
// developer config; passes with a pristine `GROK_HOME`), and its
// `STAMPS_SERVED` count assertion is racy under full-suite parallelism.
#[test]
#[ignore = "env-sensitive (ambient MCP config) and parallelism-flaky; passes isolated with pristine GROK_HOME"]
...
// fork patch 4: expects a tier re-check to be spawned and claimed; patch 4's
// fail-open short-circuits `enforce_grok_code_access()` (no remote verdict
// can ever arrive with fetch disabled), so no re-check is ever spawned.
#[test]
#[ignore = "tier re-check never spawns under local fork patch 4 fail-open"]
...
#[serial_test::serial]
// REMOTE FETCH HARD-DISABLED (local fork patch): remote settings never arrive
// on this build, so the writeback upgrade / settings-arrival emission this
// test exercises cannot happen. Behavior intentionally changed.
#[ignore = "remote fetch hard-disabled by local fork patch"]
...
#[serial_test::serial]
// REMOTE FETCH HARD-DISABLED (local fork patch): remote settings never arrive
// on this build, so the settings-arrival push/gate this test exercises cannot
// happen. Behavior intentionally changed.
#[ignore = "remote fetch hard-disabled by local fork patch"]
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[serial_test::serial]
// REMOTE FETCH HARD-DISABLED (local fork patch): no post-auth settings fetch
// is ever dispatched on this build, so the fetch-failure lifecycle this test
// exercises cannot happen; the gate fails open instead. Behavior
// intentionally changed.
#[ignore = "remote fetch hard-disabled by local fork patch"]
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[serial_test::serial]
// REMOTE FETCH HARD-DISABLED (local fork patch): remote settings are never
// fetched on this build, so the token-rotation self-heal re-fetch this test
// exercises cannot happen. Behavior intentionally changed.
#[ignore = "remote fetch hard-disabled by local fork patch"]
