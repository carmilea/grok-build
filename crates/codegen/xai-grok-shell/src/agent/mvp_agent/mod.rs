...
impl MvpAgent {
    ...
    pub(super) async fn enforce_grok_code_access(
        ...
        // REMOTE FETCH HARD-DISABLED (local fork patch): remote settings can
        // never arrive on this build, so no `allow_access` verdict will ever
        // be resolved for this identity. Fail open without consulting
        // `cfg.remote_settings`: a stale on-disk cache (written by an
        // unpatched build or before the patch) must not block the user, and a
        // cached deny verdict must not leak across identities. This also keeps
        // the blocked-path `retry_subscription_check` poll from firing.
...
pub(crate) fn settings_allow_access(
    ...
    // REMOTE FETCH HARD-DISABLED (local fork patch): this is the chokepoint
    // both writers of `tier_allowed` consult — `enforce_grok_code_access`
    // and `on_remote_settings_changed`. With remote settings unreachable, no
    // `allow_access` verdict can ever arrive, yet a stale cached
    // `Some(false)` (written by an unpatched build) would block the user
    // forever via either writer. Fail open unconditionally. Reverting this
    // re-arms the remote kill-switch.
    let _ = rs;
    true
}
mod replay;
