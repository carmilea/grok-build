...
#[test]
#[serial]
// TELEMETRY HARD-DISABLED (local fork patch): `resolve_telemetry_mode` now
// always returns `Disabled`, so "telemetry on" can no longer be reached and
// trace-upload no longer defaults on. Behavior intentionally changed.
...
// fork patch 1: asserts upstream's `DISABLE_TELEMETRY` env tier in
// `resolve_telemetry_mode()`; the fork replaces the whole resolution order
// with `Disabled`, so this test cannot pass here.
#[test]
#[serial]
#[ignore = "asserts remote-tier telemetry resolution dropped by local fork patch 1"]
