...
mod tests {
    ...
    #[test]
    #[serial_test::serial]
    // REMOTE FETCH HARD-DISABLED (local fork patch): should_open_at_startup
    // intentionally fails OPEN when resolve_remote_fetch_enabled() is false
    // ("no remote policy will arrive"), so the gate this test expects to stay
    // closed opens. Only affects the user's own external OTEL collector — all
    // xAI telemetry channels stay hard-disabled by resolve_telemetry_mode().
