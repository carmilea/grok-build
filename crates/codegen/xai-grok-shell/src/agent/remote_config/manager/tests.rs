...
#[tokio::test]
// REMOTE FETCH HARD-DISABLED (local fork patch): spawn_catalog_retry_with_backoff
// early-returns when resolve_remote_fetch_enabled() is false, so the retry this
// test waits for never spawns. Behavior intentionally changed.
...
#[tokio::test]
// REMOTE FETCH HARD-DISABLED (local fork patch): the watcher's refetch branch
// is gated on resolve_remote_fetch_enabled(), which this fork forces to false,
// so no re-fetch can happen. Behavior intentionally changed.
#[ignore = "remote fetch hard-disabled by local fork patch"]
...
// REMOTE FETCH HARD-DISABLED (local fork patch): the background models
// refresh is gated on the remote-fetch switch, so it is never dispatched on
// this build. Behavior intentionally changed.
#[ignore = "remote fetch hard-disabled by local fork patch"]
...
// fork patch 2: asserts live remote-catalog fetch/caching behavior; the
// fork hard-disables remote fetch, so no commit ever lands.
#[test]
#[ignore = "requires remote catalog fetch, disabled by local fork patch 2"]
...
// fork patch 2: asserts live remote-catalog fetch behavior; the fork
// hard-disables remote fetch, so the catalog is abandoned by design.
#[test]
#[serial]
#[ignore = "requires remote catalog fetch, disabled by local fork patch 2"]
