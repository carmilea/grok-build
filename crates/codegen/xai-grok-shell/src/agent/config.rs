...
impl Config {
    ...
    pub(crate) fn resolve_telemetry_mode(&self) -> Resolved<TelemetryMode> {
        // TELEMETRY HARD-DISABLED (local fork patch).
        //
        // This is the single source of truth that every telemetry channel gates
        // on: Mixpanel product analytics, the internal product-events API, the
        // internal OTEL span pipeline, session-metrics lifecycle events, and
        // session-trace bulk uploads. Returning `Disabled` unconditionally
        // neutralizes all of them.
        //
        // Critically, this ignores the original resolution order — requirement
        // pins, `GROK_TELEMETRY_ENABLED`, `[features] telemetry`, and
        // server-pushed `remote_settings.telemetry_*`. That server-push path was
        // the one way telemetry could be activated without a local opt-in; it is
        // now inert. Telemetry cannot be re-enabled without editing this function.
        Resolved::new(TelemetryMode::Disabled, ConfigSource::Default)
    }
    /// EGRESS-AUDIT (fork): session-trace upload gate. **Soft gate — read this
    /// before touching it.**
    ///
    /// This is the master switch for the per-turn artifact upload engine
    /// (`src/upload/*`) and for `grok trace <id>`. What it uploads is the whole
    /// session directory: `chat_history.jsonl`, `system_prompt.txt`,
    /// `prompt_context.json`, `events.jsonl` — i.e. full conversation content,
    /// including any file the agent read. Treat it as the highest-value
    /// exfiltration path in the tree.
    ///
    /// Unlike `resolve_telemetry_mode()` this does NOT hard-return `false`.
    /// It is off by default because `.default()` derives from the (always
    /// `Disabled`) telemetry mode, and the server-pushed `feature_flag` is
    /// forced to `None` on that same branch — so xAI cannot switch it on.
    /// But an explicit LOCAL opt-in still works, by design:
    ///   - `GROK_TELEMETRY_TRACE_UPLOAD=1`
    ///   - `[telemetry] trace_upload = true` in config.toml
    ///   - a `requirements.trace_upload` pin
    ///
    /// Deliberate: it preserves `--local` trace export debugging and a
    /// self-hosted bucket. If you want it welded shut, return
    /// `Resolved::new(false, ConfigSource::Default)` here and update
    /// `scripts/audit-egress.sh` + SECURITY-EGRESS.md to match.
    ///
    /// Verified by: `make audit` ("trace_upload defaults to telemetry mode").
...
pub fn is_error_reporting_disabled_sync() -> bool {
    // ERROR REPORTING HARD-DISABLED (local fork patch).
    //
    // This gate is deliberately independent of `resolve_telemetry_mode()`
    // upstream: it reads `GROK_ERROR_REPORTING`, `DISABLE_ERROR_REPORTING`,
    // and `[diagnostics] error_reporting`, and inherits the env/TOML
    // telemetry slice — so the patch-1 hard-disable never reached it, and an
    // env var or config key could arm Sentry crash reporting even with
    // telemetry hard-disabled. This fork reports nothing: return `true`
    // (disabled) unconditionally. The original chain is preserved below,
    // unreachable, so the helpers stay live for anyone who reverts this
    // patch. See PATCH.md patch 7.
    let error_reporting_disabled = true;
    if error_reporting_disabled {
        return true;
    }
...
pub fn resolve_external_otel_config(
    ...
    // EXTERNAL OTEL HARD-DISABLED (local fork patch).
    //
    // The external OTEL stream (OTLP logs/metrics export over HTTP, gRPC, and
    // mTLS, with optional prompt-text and tool-detail content) is documented
    // upstream as "independent of `TelemetryMode`" — so the telemetry
    // hard-disable (patch 1) does NOT cover it. Its enable paths are a
    // requirement pin, `GROK_EXTERNAL_OTEL`, or `[telemetry] otel_*` config —
    // the last of which a managed-config sync could previously server-push
    // (patch 6 closes that). Returning `None` here means tracing init builds
    // no external pipeline; nothing can activate the stream without editing
    // this function. The original resolution is preserved below, unreachable,
    // so the helpers stay live for anyone who reverts this patch.
    let external_otel_disabled = true;
    if external_otel_disabled {
        let _ = client;
        return None;
    }
