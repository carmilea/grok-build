...
// FORK PATCH 9 (web search backends): wire-format enum so the same web_search tool
// can speak OpenAI/xAI Responses, Moonshot/Kimi chat-completions builtin search, and
// Anthropic messages server-tool shapes.
/// Which upstream protocol the in-process `web_search` tool speaks.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum WebSearchWireFormat {
    /// OpenAI/xAI Responses API (`/v1/responses`).
    #[default]
    Responses,
    /// Moonshot/Kimi chat-completions builtin function (`$web_search`).
    MoonshotBuiltin,
...
pub enum WebSearchConfig {
    ...
    Enabled {
        ...
        // FORK PATCH 9 (web search backends): dispatch key for the request/response
        // wire format. Defaults to `Responses` so existing serialized specs keep working.
        #[serde(default)]
...
mod tests {
    ...
    fn test_config_redacted() {
        ...
        match redacted {
            WebSearchConfig::Enabled {
                ...
                assert_eq!(wire_format, WebSearchWireFormat::MoonshotBuiltin);
            }
    ...
    fn test_config_deserialize_from_set_options_payload() {
        ...
        assert!(config.is_enabled());
        // Existing payloads without `wire_format` must default to Responses.
        match config {
            WebSearchConfig::Enabled { wire_format, .. } => {
                assert_eq!(wire_format, WebSearchWireFormat::Responses);
            }
            _ => panic!("Expected Enabled variant"),
        }
    }
