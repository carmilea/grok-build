...
pub fn build_messages_request(req: &ConversationRequest) -> crate::messages::MessagesRequest {
    ...
    for item in &req.items {
        match item {
            ...
            // `thinking`.
            //
            // Provenance guard: Anthropic validates every `thinking` block's
            // cryptographic `signature` (even when thinking is disabled on the
            // request) and rejects any block whose signature is empty or not
            // one it issued. Reasoning produced by a non-Anthropic backend
            // (e.g. Moonshot/Kimi `reasoning_content`) carries no signature, so
            // replaying it here after a mid-session model switch yields a 400
            // `Invalid signature in thinking block`. Only Anthropic-signed
            // reasoning is safe to replay; drop the rest (Anthropic accepts the
            // turn fine without it). A genuine Anthropic signature validates
            // across Claude models, so Claude<->Claude switching is preserved.
