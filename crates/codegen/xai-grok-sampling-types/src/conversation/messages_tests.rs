...
fn test_messages_request_cache_breakpoint_skips_thinking() {
    // Local fork patch: the reasoning item must carry an Anthropic signature
    // (`encrypted_content`) to survive the provenance guard in
    // `build_messages_request` — unsigned reasoning is now dropped.
    let req = ConversationRequest::from_items(vec![
        ConversationItem::user("Fix the bug"),
        ConversationItem::Reasoning(crate::rs::ReasoningItem {
            encrypted_content: Some("test-anthropic-signature".to_string()),
            ..synthesized_reasoning_item("weighing options")
        }),
