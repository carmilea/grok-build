...
mod tests {
    ...
    #[test]
    fn messages_request_drops_unsigned_reasoning_keeps_signed() {
        use crate::messages::{ContentBlock, Message, MessageContent, MessageRole};
        use crate::rs;

        fn assistant_blocks(req: &ConversationRequest) -> Vec<ContentBlock> {
            build_messages_request(req)
                .messages
                .into_iter()
                .filter(|m: &Message| matches!(m.role, MessageRole::Assistant))
        ...
        // Unsigned reasoning (Moonshot/Kimi-style `reasoning_content`, no Anthropic
        // signature) must be dropped — replaying it to Anthropic after a mid-session
        // model switch yields a 400 `Invalid signature in thinking block`.
        let req = ConversationRequest::from_items(vec![
            ConversationItem::user("hi"),
            ConversationItem::Reasoning(synthesized_reasoning_item("kimi thoughts")),
            ConversationItem::assistant("answer"),
        ])
        .with_model("claude-opus-4-8");
        let blocks = assistant_blocks(&req);
        assert!(
            !blocks
                .iter()
                .any(|b| matches!(b, ContentBlock::Thinking { .. })),
            "unsigned reasoning must not emit a thinking block; got {blocks:?}",
        );
        assert!(
            blocks
                .iter()
                .any(|b| matches!(b, ContentBlock::Text { text, .. } if text == "answer")),
            "assistant text must survive dropping the reasoning; got {blocks:?}",
        );

        // Signed reasoning (genuine Anthropic thinking) is preserved with its
        // signature, so same-model and Claude<->Claude multi-turn keep working.
        let signed = rs::ReasoningItem {
            encrypted_content: Some("real-anthropic-signature".to_string()),
            ..synthesized_reasoning_item("claude thoughts")
        };
        let req = ConversationRequest::from_items(vec![
            ConversationItem::user("hi"),
            ConversationItem::Reasoning(signed),
            ConversationItem::assistant("answer"),
        ])
        .with_model("claude-opus-4-8");
        let blocks = assistant_blocks(&req);
        assert!(
            blocks.iter().any(|b| matches!(
                b,
                ContentBlock::Thinking { signature, .. } if signature == "real-anthropic-signature"
            )),
            "signed reasoning must emit a thinking block carrying its signature; got {blocks:?}",
        );
    }
