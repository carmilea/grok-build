...
pub fn init(
    ...
    // TELEMETRY HARD-DISABLED (local fork patch): ignore the requested mode and
    // never install a live client, so `track`/`sync_profile`/session metrics all
    // stay no-ops regardless of how this is called.
    let _ = mode;
...
pub fn init_if_needed(
    ...
    // TELEMETRY HARD-DISABLED (local fork patch): always treat as disabled.
    let _ = mode;
    let mode = TelemetryMode::Disabled;
    if mode.is_disabled() {
        return;
...
mod tests {
    ...
    #[test]
    // TELEMETRY HARD-DISABLED (local fork patch): `init`/`init_if_needed` now
    // never install a client, so SessionMetrics mode no longer produces a live
    // client for this test to observe. Behavior intentionally changed; see
    // `init` above and `Config::resolve_telemetry_mode` in xai-grok-shell.
