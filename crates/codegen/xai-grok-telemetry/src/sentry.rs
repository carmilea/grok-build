...
pub fn init(config: Config) -> ClientInitGuard {
    // TELEMETRY HARD-DISABLED (local fork patch): force the no-op guard so Sentry
    // crash/panic reporting never initializes, regardless of the host `Config` or
    // a runtime/build-baked `SENTRY_DSN`.
    let config = Config {
        disabled: true,
        ..config
    };
