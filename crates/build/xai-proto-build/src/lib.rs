...
impl XaiProtoBuilder {
    ...
    pub fn compile_protos(
        ...
        // WINDOWS DEPFILE SKIP (local fork fix): emit_rerun_if_changed invokes
        // protoc with --dependency_out=/dev/stdout and
        // --descriptor_set_out=/dev/null — device paths that do not exist on
        // Windows, so protoc fails and the whole build dies. The pass only
        // emits cargo rerun-if-changed hints for transitive .proto imports;
        // skipping it does not affect codegen, and on CI (fresh checkout)
        // the hints are unused. Reverting breaks every Windows cargo build
        // of xai-grok-tools-api. Report upstream.
        #[cfg(not(windows))]
